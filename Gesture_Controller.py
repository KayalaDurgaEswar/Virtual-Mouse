...
class HandRecog:
    ...
    def __init__(self, hand_label):
        ...
        ----------
            finger : int
                Represent gesture cor
                Represent gesture corresponding to Enum 'Gest',
                stores gesture being used.responding to Enum 'Gest',
...
class Controller:
    ...
    def changesystembrightness():
        ...
        # Print the type and value of the return to debug
        brightness_value = sbcontrol.get_brightness(display=0)
        print(f"Brightness value: {brightness_value} (Type: {type(brightness_value)})")

        # Handle if brightness_value is a list
        if isinstance(brightness_value, list):
            # Assuming the list contains a single element; adjust based on your needs
            brightness_value = brightness_value[0] if brightness_value else 0
        else:
            # Handle unexpected types or raise an error
            raise ValueError("Expected brightness to be a list or a numeric value")

        # Proceed with existing code
        current_brightness = brightness_value / 100.0
        adjustment = Controller.pinchlv / 50.0
        new_brightness = current_brightness + adjustment
        new_brightness = max(0.0, min(1.0, new_brightness))
        sbcontrol.fade_brightness(int(100 * new_brightness), start=int(100 * current_brightness))
